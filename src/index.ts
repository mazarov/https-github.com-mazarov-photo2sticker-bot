...
/**
 * Get example sticker generated by AI assistant (selected_style_id = 'assistant').
 */
async function getAssistantStyleExample(): Promise<{ telegram_file_id: string } | null> {
  const { data } = await supabase
    .from("stickers")
...
/**
 * Handle show_style_examples tool — send example stickers or style buttons to user.
 */
  ...
  if (styleId) {
    ...
    if (styleId === "assistant") {
      ...
      if (example?.telegram_file_id) {
        try { await ctx.replyWithSticker(example.telegram_file_id); } catch {}
      } else {
        await ctx.reply(isRu ? "Примеров от ассистента пока нет." : "No assistant examples yet.");
      }
      return;
    }
    ...
    // === Show buttons for ALL active styles + assistant ===
