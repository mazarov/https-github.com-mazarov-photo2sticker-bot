...
async function runPackPreviewJob(job: any) {
  ...
  // Style prompt: agent's prompt_final (main logic) + raw preset hint so selected style is explicit.
  // 1) prompt_final from session (generated by shared prompt_generator agent)
  // 2) fallback or reinforcement: when selected_style_id is set, add preset.prompt_hint so style matches selection
  ...
  let selectedStyleHint = promptFinalRaw;
  ...
  if (session.selected_style_id) {
    ...
    if (stylePreset) {
      ...
      const rawHint = (stylePreset.prompt_hint || "").trim();
      if (rawHint) {
        if (!selectedStyleHint) {
          selectedStyleHint = rawHint;
          console.log("[PackPreview] Using style from preset (no prompt_final):", stylePreset.id, selectedStyleName);
        } else {
          selectedStyleHint = `${selectedStyleHint}\n[Exact style reference, must match:] ${rawHint}`;
          console.log("[PackPreview] Using prompt_final + preset hint:", stylePreset.id, selectedStyleName);
        }
      }
    }
  }
  if (selectedStyleHint && !selectedStyleName) {
    console.log("[PackPreview] Using style prompt from session.prompt_final");
  }
