...
3. Клик "💡 Идеи для пака":
   - AI (Gemini Flash) получает: стикер-изображение + стиль + контекст
...
### Модель
**Gemini 2.5 Flash** (текстовый запрос + анализ изображения) — быстрый, дешёвый.
...
- [ ] Функция `generatePackIdeas()` — вызов Gemini Flash с изображением стикера
