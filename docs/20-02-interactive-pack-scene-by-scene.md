...
### 5.4. Critic и валидация в интерактиве

В интерактивном режиме **LLM Critic не в hot-path** (не после каждой сцены). Вместо этого:

- **Code-level validators** после каждой сгенерированной сцены/подписи (без LLM):
  - в scene_description есть ровно один `{subject}` (regex);
  - длина подписи 15–20 символов (RU/EN);
  - наличие и RU, и EN подписи.
- При нарушении — сразу Regenerate по правилам (см. раздел 8), без вызова агента. Так узнаём о проблеме сразу, а не «слишком поздно».

**Critic как агент:** только опциональный финальный gate по собранному spec (9 сцен + 9 подписей) перед сохранением — по желанию продукта.
...
interface PackDNA {
  ...
  // Moment pool: 9 элементов с приоритетом по сложности (не показывается юзеру)
  moment_pool: { text: string; risk: "safe" | "medium" | "awkward" }[];
...
**Scene difficulty / risk (продуктовая страховка):** у каждого момента в pool есть `risk`. Правило порядка генерации: **сцены 1–2** — только моменты с `safe` или `medium`; **awkward** — не раньше 3-й сцены. Так первый стикер не окажется «слишком странным», снижаем риск early churn. Архитектуру это не усложняет.
...
- `one_moment`: string (один элемент из moment_pool; при выборе учитываем risk и порядок — см. раздел 7.2)
...
**Debounce для interactive_update (логика, не агент):** чтобы не дергать Brief & Plan на каждый Adjust taste и не тратить токены впустую:
- **Вариант A:** Taste Delta применяется **накопительно** в памяти; вызов `interactive_update` — не чаще чем раз в 2 сцены (или после N сцен).
- **Вариант B:** применять delta только после **явного подтверждения** пользователя («Применить» / следующая сцена).
- Итог: меньше вызовов, меньше «дёргания» вкуса туда-сюда.

### 7.2. Used moments и порядок по risk

- Массив индексов или текстов моментов из `moment_pool`, уже закоммиченных.
- **Правило выбора следующего момента:** сцены 1–2 — только моменты с `risk: "safe"` или `"medium"`; моменты с `risk: "awkward"` допускаются начиная с 3-й сцены. Внутри одной «волны» — по порядку или по приоритету (например, сначала все safe, потом medium, потом awkward).
