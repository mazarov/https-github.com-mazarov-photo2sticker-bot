...
| `SUPABASE_STORAGE_BUCKET_EXAMPLES` | `sticker-examples` | Публичный бакет для примеров стилей (лендинг). Создать в Storage, включить **Public**. При «Сделать примером» файл копируется сюда и в `stickers.public_url` пишется URL. |
