...
## Edit Sticker / Replace Subject additions

Миграция `123_edit_sticker_replace_subject.sql` добавляет:

- `ALTER TYPE session_state ...`
  - `wait_edit_sticker`
  - `wait_edit_action`
  - `wait_edit_photo`
- `stickers.generation_type text` — хранит тип генерации (`style|emotion|motion|text|replace_subject|imported`).
- `sessions.edit_replace_sticker_id uuid` — временный `stickers.id` для callback `replace_face`.
- `sessions.edit_sticker_file_id text` — временный `telegram_file_id` внешнего стикера в edit-flow.
