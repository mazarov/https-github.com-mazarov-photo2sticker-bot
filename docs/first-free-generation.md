...
- [ ] Создать миграцию `sql/027_first_free_generation.sql`
- [ ] Добавить `is_first_free` в таблицу `jobs`
- [ ] Обновить `index.ts` — проверка first free перед кредитами
- [ ] Обновить `worker.ts` — выбор модели + инкремент счётчика
- [ ] Деплой
