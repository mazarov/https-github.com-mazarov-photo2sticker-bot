# Бизнес-уведомления (Business Notifications)

## Цель

Получать уведомления о ключевых событиях в Telegram-канал для мониторинга активности бота.

## Типы уведомлений

| Тип | Emoji | Когда |
|-----|-------|-------|
| `new_user` | 👤 | Новый пользователь зарегистрировался |
| `new_sticker` | 🎨 | Стикер успешно сгенерирован |
| `new_payment` | 💰 | Оплата прошла успешно |

## Формат сообщений

### Новый пользователь
```
👤 Новый пользователь

@username (42269230)
🌐 Язык: ru
```

### Новый стикер (с картинкой)
```
🎨 Новый стикер

👤 @username (42269230)
🎭 Тип: style (anime)

[Стикер прикреплён как изображение]
```

### Новая оплата
```
💰 Новая оплата

👤 @username (42269230)
📦 Пакет: 30 кредитов
⭐ Сумма: 150 Stars
```

## Конфигурация

Используем тот же канал что и для ошибок:
```env
ALERT_CHANNEL_ID=-100xxx
```

## Реализация

### Принцип: не блокировать основной флоу

Все уведомления отправляются **без await** — не влияют на скорость ответа пользователю:

```typescript
sendNotification(...).catch(err => console.error("Notification failed:", err));
```

### Модуль: `src/lib/alerts.ts`

Добавить новые типы и функцию для отправки фото:

```typescript
type NotificationType = "new_user" | "new_sticker" | "new_payment";

async function sendNotification(options: {
  type: NotificationType;
  message: string;
  imageBuffer?: Buffer;  // Для стикера
}): Promise<void>;
```

### Точки интеграции

| Файл | Место | Тип |
|------|-------|-----|
| `index.ts` | После создания пользователя в /start | `new_user` |
| `worker.ts` | После успешной генерации стикера | `new_sticker` |
| `index.ts` | После successful_payment | `new_payment` |

### index.ts — new_user

```typescript
// После создания нового пользователя
if (isNewUser && user) {
  sendNotification({
    type: "new_user",
    message: `@${ctx.from?.username || "no_username"} (${telegramId})\n🌐 Язык: ${lang}`,
  }).catch(console.error);
}
```

### worker.ts — new_sticker

```typescript
// После отправки стикера пользователю
sendNotification({
  type: "new_sticker",
  message: `👤 @${user.username || user.telegram_id}\n🎭 Тип: ${generationType}`,
  imageBuffer: stickerBuffer,
}).catch(console.error);
```

### index.ts — new_payment

```typescript
// После успешной оплаты
sendNotification({
  type: "new_payment",
  message: `👤 @${user.username || telegramId}\n📦 Пакет: ${amount} кредитов\n⭐ Сумма: ${price} Stars`,
}).catch(console.error);
```

## Checklist

- [x] Обновить `src/lib/alerts.ts` — добавить `sendNotification` с поддержкой изображений
- [x] Интегрировать в `index.ts` — new_user при /start
- [x] Интегрировать в `worker.ts` — new_sticker после генерации
- [x] Интегрировать в `index.ts` — new_payment после оплаты
- [ ] Тестирование всех трёх типов уведомлений
