...
- **Уточнение:** пак по контенту (content set) в боте может быть сгенерирован в **любом** стиле (пользователь выбирает стиль из `style_presets_v2`). Это не обязательно photo_realistic. Для Hero на кластерных страницах отображаемый стиль задаётся пресетом, у которого путь страницы входит в `landing_hero_paths` (см. п. 2.2); при «сделать примером» для content set обычно берут пак, сгенерированный в этом же стиле.
...
В **`style_presets_v2`** используется колонка **`landing_hero_paths`** (text[], default '{}').

- В массиве — **полные пути** страниц, на которых в Hero показывать пак этого пресета (pack/style/{id}/1..9). Пустой массив = нигде не показывать.
- Один путь должен быть только у одного пресета. Резолв: для текущего path выбирается пресет, у которого `path = any(landing_hero_paths)` (при нескольких — первый по sort_order).
- На кластерных страницах переключение по пилюлям меняет пак (content set); стиль Hero на такой странице задаётся пресетом, в чьём `landing_hero_paths` лежит путь страницы (например '/' или '/style').
...
- **Карусель:** 9 стикеров выбранного пака. Стиль = пресет, у которого путь страницы в **`landing_hero_paths`**. Файлы в Storage: на кластерных — пак по **content_set_id** (pack/content/…); на страницах стилей — pack/style/{preset_id}/1..9 (см. п. 6).
- Сменить стиль Hero на странице = добавить/убрать путь в `landing_hero_paths` нужного пресета; в UI на кластерных переключается только пак (пилюля).
...
|-----|-----|-------------------|
| Пак по content set | `pack/content/{content_set_id}/1.webp` … `9.webp` | Hero на кластерных: пилюли = content sets с cluster=true, стиль = пресет, у которого путь в landing_hero_paths. |
...
- **Кластерные:** карусель = `pack/content/{content_set_id}/1..9.webp`, где content_set_id выбранной пилюли; стиль задаётся пресетом, у которого путь страницы в `landing_hero_paths`.
...
|-------|------|----------------|
| Кластерные (/, /style, /bot, …) | Паки с `cluster = true`, стиль = пресет, у которого путь в `landing_hero_paths`; пилюли = эти паки. | Все пресеты с `landing = true` по sort_order; карточка = пресет, превью = 1.webp пака этого пресета. |
...
2. **style_presets_v2:**  
   - **`landing_hero_paths`** (text[], default '{}'). Полные пути страниц, на которых в Hero показывать пак этого пресета (pack/style/{id}/1..9). Один путь — только у одного пресета. Резолв по path = any(landing_hero_paths).  
