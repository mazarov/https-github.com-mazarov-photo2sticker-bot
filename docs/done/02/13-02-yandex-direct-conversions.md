...
### Переотправка ранее сделанных конверсий (вариант Б)

Если конверсии уже отправлялись с неверным форматом (колонка `UserId` вместо `yclid`) и не привязались в Метрике, можно переотправить их скриптом за последние **21 день** (окно привязки Метрики):

1. **Сухой прогон** (только список транзакций):
   ```bash
   npm run retry:yandex-conversions:dry
   ```
2. **Выполнить переотправку** (сброс флага `yandex_conversion_sent_at` у выбранных транзакций и отправка в Метрику с колонкой `yclid`):
   ```bash
   npm run retry:yandex-conversions
   ```

Скрипт: `scripts/retry-yandex-conversions.ts`. Выбирает транзакции: `created_at` за последние 21 день, `price > 0`, `yandex_conversion_sent_at IS NOT NULL`, у пользователя есть `yclid` (или извлекается из `start_payload`). Сбрасывает флаг, отправляет конверсии через `sendYandexConversion`, обновляет `yandex_conversion_sent_at` при успехе.
