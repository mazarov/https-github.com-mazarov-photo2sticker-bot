...
Колонки `show_in_onboarding` контролируют видимость для пользователей в процессе онбординга (`onboarding_step < 2`):

| Таблица | Что скрывает |
|---------|--------------|
| `style_groups` | Вся группа целиком |
| `style_presets_v2` | Конкретный подстиль |

- `true` (default) — видно всем
- `false` — скрыто для новичков, видно после завершения онбординга

**Примеры:**
```sql
-- Скрыть группу "Русские" и "Сериалы" для новичков
UPDATE style_groups SET show_in_onboarding = false WHERE id IN ('russian', 'tv');

-- Скрыть подстиль "Тёмный аниме" для новичков
