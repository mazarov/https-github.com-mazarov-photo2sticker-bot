...
|-----------|---------------------|
| **Landing** (сабмодуль `/landing`) | Capture + Transport: собирает `utm_*` + `yclid` из URL, формирует deep link. События в Метрику **не** шлёт |
| **photo2sticker-bot** (`src/`) | Storage + Dispatch: парсит payload, сохраняет `yclid` в `users`, отправляет offline conversion в Метрику **только по факту оплаты** |
| **Яндекс Директ** | Оптимизируется по целям Метрики (офлайн-конверсии: `purchase`, `purchase_try`, `purchase_start` и т.д.) |
...
**Файл:** `landing/client/src/components/landing/TelegramButton.tsx` — только ссылка на deep link, без вызовов Метрики (события только из бота по факту оплаты).
...
// Returning user: только если yclid ещё пустой (first-click — атрибуция по первому переходу)
if (startPayload && utm.yclid && !user.yclid) {
...
### Этап 1: Лендинг — передача yclid в deep link (0.5 дня)

Лендинг уже существует (`/landing` — git submodule), Метрика подключена (ID `106534984`). События в Метрику с лендинга **не отправляются** — только из бота по факту оплаты.
...
**Definition of Done:** deep link содержит yclid.
...
- [ ] Обновлять `yclid` для returning users только если пустой (first-click)
...
- [ ] Настроить автостратегию "Оптимизация конверсий" по офлайн-целям (`purchase`, `purchase_try`, `purchase_start` и т.д.)
...
События по факту оплаты отправляются из бота в Метрику с полем **Target** = идентификатор цели. Эти цели нужно **создать вручную** в счётчике, иначе загрузка вернёт ошибку: «В Яндекс Метрике не найдены указанные в файле идентификаторы целей».

### Шаг 1: Открыть настройки целей

1. Зайти в [Яндекс Метрику](https://metrika.yandex.ru/).
2. Выбрать счётчик **106534984** (лендинг photo2sticker).
3. В левом меню: **Настройка** → **Цели** (или **Отчёты** → **Настройка** → вкладка **Цели**).

### Шаг 2: Создать цели типа «JavaScript-событие»

Для каждой цели:

1. Нажать **Добавить цель**.
2. **Тип цели:** выбрать **JavaScript-событие**.
3. **Идентификатор цели** — ввести **точно** одно из значений из таблицы ниже (без пробелов, без опечаток).
4. **Название цели** — любое понятное имя для отчётов (например, «Покупка — Попробовать»).

Создать **6 целей** с такими параметрами:

| Идентификатор цели (обязательно так) | Название цели (для отчётов) | Когда срабатывает |
|--------------------------------------|-----------------------------|-------------------|
| `purchase` | Покупка (прочее) | Скрытые/скидочные пакеты, неизвестный пакет |
| `purchase_try` | Покупка — Попробовать | Trial 5+5 кредитов |
| `purchase_start` | Покупка — Старт | 10 кредитов |
| `purchase_pop` | Покупка — Поп | 30 кредитов |
| `purchase_pro` | Покупка — Про | 100 кредитов |
| `purchase_max` | Покупка — Макс | 250 кредитов |

Важно: в колонке **«Идентификатор цели»** в интерфейсе Метрики должно быть введено **ровно** `purchase`, `purchase_try`, `purchase_start`, `purchase_pop`, `purchase_pro`, `purchase_max` — так же, как в коде бота (`src/lib/yandex-metrika.ts`).

### Шаг 3: Проверка

- В списке целей счётчика должны быть все 6 целей с типами «JavaScript-событие».
- После первой успешной оплаты с yclid в отчётах Метрики (через 1–2 часа) появятся офлайн-конверсии с этими целями и суммой (Price в RUB).
...
Цели используются для отчётов и для **оптимизации кампаний Яндекс.Директа** (автостратегия «Оптимизация конверсий»).
...
|------|-------------|-----------|
| Мало конверсий для автостратегии (<10/нед) | Высокая на старте | Накопить конверсии; оптимизация по офлайн-целям (purchase_*) |
